...
const migration = readFileSync('supabase/migrations/20260825171500_public_leaflets_documents_only.sql', 'utf8');
...
assert.match(migration, /s\.slug = 'globus'[\s\S]*globus\.cz/i,
  'Globus must remain eligible only through its explicit document resolver boundary.');
...
console.log('Public leaflet catalogue is constrained to renderable documents.');
