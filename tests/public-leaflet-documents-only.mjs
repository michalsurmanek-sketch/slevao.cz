...
const migration = readFileSync('supabase/migrations/20260825174500_require_direct_globus_document.sql', 'utf8');
...
assert.doesNotMatch(migration, /s\.slug = 'globus'[\s\S]*store:globus-html/i,
  'Uncached Globus HTML discovery imports must not be public leaflet documents.');
...
console.log('Public leaflet catalogue exposes only renderable document-backed sources.');
