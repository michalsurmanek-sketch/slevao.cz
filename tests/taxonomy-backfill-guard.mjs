...
const sql = fs.readFileSync(new URL('../supabase/migrations/20260816231500_guarded_taxonomy_backfill.sql', import.meta.url), 'utf8');

assert.match(sql, /p_limit < 1 or p_limit > 500/i, 'Backfill batch size must stay bounded.');
...
assert.match(sql, /revoke all on function private\.apply_product_taxonomy_candidates.*anon, authenticated/is, 'Backfill function must not be callable by browser roles.');
assert.match(sql, /grant execute on function private\.apply_product_taxonomy_candidates.*service_role/is, 'Backfill function must be service-role only.');
