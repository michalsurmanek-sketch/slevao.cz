...
assert(sync.includes("const ADAPTER = 'tesco-apollo-pdf-v15-safe-public';"), 'Tesco safe adapter revision must stay explicit');
...
assert(sync.includes('(semantic === 1 && layout <= 100)'), 'Tesco sync must keep one-word evidence tight');
assert(sync.includes('(mathUnique && spatial <= 150)'), 'Tesco sync must bound math-only evidence spatially');
assert(sync.includes("if (rows.length < 20 || rows.length > 80)"), 'Tesco sync must fail closed outside the safe row range');
...
assert(sync.includes("price_kind: 'public'"), 'Tesco v15 must publish public prices only');
assert(sync.includes("p_min_products: 20"), 'Publisher minimum must match safe snapshot contract');
assert(sync.includes("p_max_products: 80"), 'Publisher maximum must match safe snapshot contract');
...
assert(probe.includes('function mathEvidence(h:any,c:C)'), 'Probe must validate quantity/unit-price math');
assert(probe.includes('unique:count===1'), 'Math evidence must be unique on the page');
...
console.log('Tesco Apollo/PDF safe product pipeline contract OK');
