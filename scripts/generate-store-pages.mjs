...
const STORE_FEED_VERSION = '20260822-1';
...
<footer><div class="c"><strong>SLEVAO.cz</strong><p>Platné slevy českých obchodů na jednom místě.</p></div></footer><script>window.SLEVAO_STORE=${storeJson}</script><script src="assets/store-feed.js?v=${STORE_FEED_VERSION}"></script><script src="${STORE_NAV_SCRIPT}"></script>
</body></html>`;
};
...
function patchExistingStorePage(pageUrl, slug) {
  ...
  const matches = current.match(navPattern) || [];
  if (matches.length !== 1) {
    throw new Error(`${slug}.html: očekáván právě jeden store-bottom-nav runtime, nalezeno ${matches.length}.`);
  }
  const next = current.replace(navPattern, STORE_NAV_SCRIPT);
