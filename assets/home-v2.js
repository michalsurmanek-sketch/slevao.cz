(() => {
  ...
  const SEARCH_EXPANSIONS = {
    ...
    lezak: ['lezak','pils','pilsner'],
    'vycepni pivo': ['vycepni'],
    'nealkoholicke pivo': ['nealkohol','0,0','0.0','birell'],
    radler: ['radler'],
    'plnotucne mleko': ['plnotuc'],
    'polotucne mleko': ['polotuc'],
    'bezlaktozove mleko': ['bezlaktoz'],
    'rostlinny napoj': ['rostlinny napoj','ovesny napoj','mandlovy napoj','sojovy napoj'],
    pecivo: ['peciv','chleb','rohlik','housk','baget','vek','dalam','kaiser','toast','koblih','croissant'],
    chleb: ['chleb'],
    rohliky: ['rohlik'],
    housky: ['housk'],
    bagety: ['baget'],
    'sladke pecivo': ['koblih','croissant','buchta','kolac','zavin','snek'],
    vejce: ['vejce','vajec'],
    'slepici vejce': ['slepic','vejce'],
    'krepelci vejce': ['krepelc'],
    'vejce m': ['vejce m','velikost m','m 10 ks','m 6 ks'],
    'vejce l': ['vejce l','velikost l','l 10 ks','l 6 ks'],
    maslo: ['maslo','maslicko'],
    'klasicke maslo': ['maslo'],
    'prepustene maslo': ['prepustene','ghi','ghee'],
    'solene maslo': ['solene maslo'],
    'ochucene maslo': ['bylinkove maslo','ochucene maslo','cesnekove maslo'],
    syr: ['syr','eidam','gouda','emental','hermelin','niva','mozzarell','cheddar','parenic','korbac','balkansky','cottage'],
    eidam: ['eidam'],
    gouda: ['gouda'],
    hermelin: ['hermelin'],
    mozzarella: ['mozzarell'],
    'taveny syr': ['taveny','tavene'],
  ...
  const COMPOUND_SEARCHES = {
    'pivo plech': [SEARCH_EXPANSIONS.pivo, ['plech']],
    'pivo lahev': [SEARCH_EXPANSIONS.pivo, ['lahev','lahv','vratna']],
    'multipack pivo': [SEARCH_EXPANSIONS.pivo, ['pack','baleni','4x','6x','8x','10x','12x','24x']],
    'cerstve mleko': [SEARCH_EXPANSIONS.mleko, ['cerstve','cerstvy']],
    'trvanlive mleko': [SEARCH_EXPANSIONS.mleko, ['trvanlive','uht']],
    'kondenzovane mleko': [SEARCH_EXPANSIONS.mleko, ['kondenzovan']],
    'cerstve pecivo': [SEARCH_EXPANSIONS.pecivo, ['cerstve','cerstvy','rozpekane']],
    'balene pecivo': [SEARCH_EXPANSIONS.pecivo, ['balene','baleny','baleni']],
    'bezlepkove pecivo': [SEARCH_EXPANSIONS.pecivo, ['bezlepk']],
    'vejce volny vybeh': [SEARCH_EXPANSIONS.vejce, ['volny','vybeh']],
    'vejce podestylka': [SEARCH_EXPANSIONS.vejce, ['podestyl']],
    'bio vejce': [SEARCH_EXPANSIONS.vejce, ['bio','ekolog']],
    'maslo kostka': [SEARCH_EXPANSIONS.maslo, ['kostka','250','200']],
    'maslo kelimek': [SEARCH_EXPANSIONS.maslo, ['kelimek','vanicka']],
    'tvrdy syr': [SEARCH_EXPANSIONS.syr, ['tvrdy','eidam','gouda','emental','cheddar']],
    'mekky syr': [SEARCH_EXPANSIONS.syr, ['mekky','hermelin','niva','mozzarell','balkansky','cottage']],
    'platkovy syr': [SEARCH_EXPANSIONS.syr, ['platk']],
    'strouhany syr': [SEARCH_EXPANSIONS.syr, ['strouhan']],
  ...
  ['lezak','vycepni pivo','nealkoholicke pivo','radler','pivo plech','pivo lahev','multipack pivo']
    .forEach((query) => { GROCERY_QUERY_CATEGORIES[query] = new Set(['drinks','food']); });
  [
    'plnotucne mleko','polotucne mleko','bezlaktozove mleko','rostlinny napoj','cerstve mleko','trvanlive mleko','kondenzovane mleko',
    'chleb','rohliky','housky','bagety','sladke pecivo','cerstve pecivo','balene pecivo','bezlepkove pecivo',
    'slepici vejce','krepelci vejce','vejce m','vejce l','vejce volny vybeh','vejce podestylka','bio vejce',
    'klasicke maslo','prepustene maslo','solene maslo','ochucene maslo','maslo kostka','maslo kelimek',
    'eidam','gouda','hermelin','mozzarella','taveny syr','tvrdy syr','mekky syr','platkovy syr','strouhany syr'
  ].forEach((query) => { GROCERY_QUERY_CATEGORIES[query] = new Set(['food']); });
  ...
  function compactQuantity(value) {
    const firstVariant = String(value || '').split(/\s*\/\s*/)[0].trim();
    return firstVariant
      .replace(/\s*=\s*/g, ' · ')
      .replace(/pracích\s+dávek/gi, 'dávek')
      .replace(/\s+/g, ' ')
      .trim();
  }

  function miniAdCard(position) {
    return `<aside class="miniAdCard" data-ad-position="${position}" aria-label="Reklamní sdělení">
      <span class="miniAdLabel">Reklama</span>
      <span class="miniAdMark" aria-hidden="true">%</span>
      <div class="miniAdContent">
        <strong>Prostor pro vaši nabídku</strong>
        <p>Oslovte zákazníky právě ve chvíli, kdy porovnávají ceny.</p>
      </div>
      <a href="kontakt.html" class="miniAdAction">Inzerovat na Slevao <span aria-hidden="true">→</span></a>
    </aside>`;
  }
  ...
  function renderDeals() {
    ...
    $('dealGrid').innerHTML = visible.map((offer, index) => {
