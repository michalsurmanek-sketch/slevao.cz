(() => {
  ...
  const date = (v) => v ? new Intl.DateTimeFormat('cs-CZ', { day:'numeric', month:'numeric', year:'numeric' }).format(new Date(v)) : '–';
  ...
  function dealLabel(offer) {
    ...
    if (!typical || !price) return { label:'Aktuální nabídka', className:'' };
  ...
  function chartSvg() {
    ...
    const points = rows.map((row, index) => {
      ...
      const y = height - pad - ((Number(row.price) - min) / span) * (height - pad * 2);
      return { x, y, row };
    });
  ...
  function offerHtml(offer, index) {
    ...
    const discount = Number(offer.old_price) > Number(offer.price) ? Math.round((Number(offer.old_price) - Number(offer.price)) / Number(offer.old_price) * 100) : 0;
    const location = leafletLocations.find((row) => row.store_id === offer.store_id);
    ...
    return `<article class="sfCard sfOffer ${index === 0 ? 'best' : ''}">
      <div class="sfOfferStore">${esc(store?.name || 'Obchod')}${index === 0 ? ' · nejlevnější' : ''}</div>
      <div><span class="sfPrice">${money(offer.price)} Kč</span>${offer.old_price ? `<span class="sfOldPrice">${money(offer.old_price)} Kč</span>` : ''}</div>
      <div class="sfMuted">${offer.unit_price ? `${money(offer.unit_price)} Kč/${esc(offer.unit_price_unit || 'jednotka')} · ` : ''}platí do ${date(offer.valid_to)}</div>
      <div style="margin-top:9px"><span class="sfBadge ${label.className}">${esc(label.label)}</span>${discount ? ` <span class="sfBadge">−${discount} %</span>` : ''}</div>
  ...
  function render() {
    const current = offers.slice().sort((a,b) => Number(a.price) - Number(b.price));
    const cheapest = current[0];
    const typical = typicalPrice();
    document.title = `${product.name} – ceny a historie | Slevao.cz`;
    document.querySelector('meta[name="description"]')?.setAttribute('content', `Porovnání aktuálních cen produktu ${product.name}, historie cen a cenový hlídač.`);
    ...
    $('currentPrice').textContent = cheapest ? `${money(cheapest.price)} Kč` : 'Bez aktuální ceny';
    $('currentStore').textContent = cheapest ? `Nejlevněji v ${cheapest.stores?.name || 'obchodě'}` : 'Aktuální nabídka není dostupná';
    ...
    $('statStores').textContent = String(new Set(current.map((row) => row.store_id)).size);
    $('offers').innerHTML = current.length ? current.map(offerHtml).join('') : '<div class="sfEmpty">Tento produkt právě nemá platnou akční nabídku.</div>';
  ...
  async function load() {
    ...
    const [productResult, offersResult, historyResult, locationResult] = await Promise.all([
      ...
      db.from('offers').select('id,product_id,store_id,title,price,old_price,unit_price,unit_price_unit,valid_from,valid_to,source_url,stores(id,name,slug)').eq('product_id', productId).eq('status','published').lte('valid_from', today).gte('valid_to', today).limit(100),
  ...
  async function reportOffer(offer) {
    ...
    const reportType = map[type.trim().toLowerCase()] || 'other';
    ...
    const { error } = await db.from('offer_reports').insert({
      ...
      report_type: reportType, note: note.slice(0,2000), page_url: location.href, status:'new'
    });
    if (error) throw error;
