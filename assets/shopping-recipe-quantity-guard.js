(() => {
  ...
  function repairName(row, name, quantity, unit) {
    if (hasRecipeProvenance(row)) {
      ...
      // Parentheses may describe the ingredient itself, e.g. "Rajčata (cherry)".
      // Never append a guessed amount behind an unknown semantic suffix.
      ...
      if (meaningfulStructuredAmount) return `${name} (${formatQuantity(quantity)} ${label})`;
    }
