...
begin
  for rec in
    select
      q.query_text,
      case when lower(public.unaccent(q.base_text)) = 'olej na smazeni' then 'Olej' else q.base_text end as search_text,
  ...
  loop
    ...
    eligible_base as (
      ...
      where not rec.is_recipe or (
        ...
        and (lower(public.unaccent(rec.search_text)) <> 'hovezi maso' or s.candidate_text !~ '(mlet|meln|burger|tatarak)')
        and (lower(public.unaccent(rec.search_text)) <> 'hladka mouka' or s.candidate_text !~ '(spald|zitn|celozrnn|bezlepk)')
        and (lower(public.unaccent(rec.search_text)) <> 'sadlo' or (s.candidate_text !~ 'bez kuze' and s.context_text !~ 'maso a ryby'))
        and (lower(public.unaccent(rec.search_text)) <> 'strouhanka' or s.candidate_text !~ '(panko|japonsk)')
        and (lower(public.unaccent(rec.search_text)) <> 'parmazan' or s.candidate_text !~ '(a la parmazan|styl parmazan)')
