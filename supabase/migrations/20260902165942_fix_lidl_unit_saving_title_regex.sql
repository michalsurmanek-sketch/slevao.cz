...
set title = public.sanitize_lidl_verified_title(
      regexp_replace(lii.title, E'\\s*[·•]\\s*([0-9]+(?:[.,][0-9]+)?\\s*(?:g|kg|ml|l|ks))\\s*$', '', 'i')
    ) || case
...
where lii.import_id = li.id
  ...
  and s.slug = 'lidl'
  and lii.title ~* E'^\\*?\\s*úspora\\s+na\\s+měrné\\s+ceně\\s+v\\s+porovnání\\s+s\\s+produktem\\s+ve\\s+standardně\\s+nabízené\\s+velikosti';
...
where o.store_id = s.id
  and s.slug = 'lidl'
  and o.title ~* E'^\\*?\\s*úspora\\s+na\\s+měrné\\s+ceně\\s+v\\s+porovnání\\s+s\\s+produktem\\s+ve\\s+standardně\\s+nabízené\\s+velikosti';
...
set name = public.sanitize_lidl_verified_title(p.name),
    ...
    updated_at = now()
where p.name ~* E'^\\*?\\s*úspora\\s+na\\s+měrné\\s+ceně\\s+v\\s+porovnání\\s+s\\s+produktem\\s+ve\\s+standardně\\s+nabízené\\s+velikosti'
  and exists (
