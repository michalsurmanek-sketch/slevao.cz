-- Source-aware classifier fallback for stores with near-pure assortments, plus a narrow furniture collision repair.
...
begin
  ...
  -- Strong non-food contexts always win.
  ...
  -- Furniture wording can contain "toaletni" but is still furniture, not drugstore.
  if n ~ '(^| )toaletni stolek( |$)' then return 'home'; end if;

  -- The word tablet alone must not decide electronics.
  ...
  -- Final store-context fallback only after every product-specific signal above.
...
set name = name
where (
    lower(coalesce(metadata->>'source_store_slug','')) in ('cropp','reserved','house','sinsay','moebelix','jysk','asko')
    or public.normalize_text(name) ~ '(^| )toaletni stolek( |$)'
  )
  and (
