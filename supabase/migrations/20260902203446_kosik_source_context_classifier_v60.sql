create or replace function public.infer_product_filter_group_kosik_context_v60(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(polevka|shirataki|tagliatelle)' then return 'food'; end if;
  if n ~ 'protein' and q ~ '(^| )([3-9][0-9]|100) g( |$)' then return 'food'; end if;
  if n ~ '(kancelarsky papir|copy paper|papir .*a4)' then return 'school'; end if;
  if n ~ ' (aa|aaa)$' and q ~ '(^| )[0-9]+ ks( |$)' then return 'electronics'; end if;
  return 'other';
end;
$function$;
...
where p.is_active is true
  ...
  and lower(trim(coalesce(p.metadata->>'source_store_slug','')))='kosik'
  and exists (
