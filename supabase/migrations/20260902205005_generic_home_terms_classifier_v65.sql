create or replace function public.infer_product_filter_group_generic_terms_v65(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(dekoracni venec|vaflovac)' then return 'home'; end if;
  return public.infer_product_filter_group_generic_terms_v54(p_name,p_quantity_text);
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 65 $function$;
...
begin
  ...
  if coalesce(nullif(trim(new.filter_group),''),'other')='other' or v_old_auto or v_new_auto then
    ...
    if v_inferred='other' then v_inferred := public.infer_product_filter_group_generic_terms_v65(new.name,new.quantity_text); end if;
    ...
    if v_inferred <> 'other' then
      ...
      elsif public.infer_product_filter_group_generic_terms_v65(new.name,new.quantity_text) <> 'other' then new.classification_source := 'generic-terms-v65';
