create table if not exists public.shopping_purchase_repeat_mutations (
  ...
  purchase_id uuid not null references public.shopping_list_purchases(id) on delete cascade,
...
create index if not exists shopping_purchase_repeat_mutations_created_idx
  on public.shopping_purchase_repeat_mutations(user_id, created_at);

alter table public.shopping_purchase_repeat_mutations enable row level security;
revoke all on table public.shopping_purchase_repeat_mutations from public;
revoke all on table public.shopping_purchase_repeat_mutations from anon;
...
set search_path = ''
as $$
  select public.repeat_shopping_purchase(p_purchase_id, extensions.gen_random_uuid());
$$;
