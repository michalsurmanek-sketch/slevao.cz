drop trigger if exists zzzzzz_safe_food_subcategory_guard_v111 on public.products;

create or replace function public.guard_safe_food_subcategory_v112()
returns trigger
language plpgsql
security invoker
...
begin
  ...
  if source_cat='OVOCE A ZELENINA' or (src='rohlik' and n ~ '(^| )(broskev|rukola)( |$)') or n ~ '(^| )hrozno bile bezsemenne( |$)' or n ~ '(^| )cerstvy mlady kokos drink and eat( |$)' then
    ...
    v_slug:='mlecne-vyrobky'; v_tag:='mlecne';
  elsif n ~ '(^| )(rybi prsty|chapadla z kalamaru|file labuznicke|susene maso|jatrovy nakroj|sendvicovy narez|paprikas|bochanek madrid)( |$)' then
    v_slug:='maso-ryby'; v_tag:='maso';
  elsif n ~ '(^| )brumik( |$)' then
  ...
  new.classification_source:='food-subcategory-guard-v112';
  new.classified_at:=now();
  new.metadata:=coalesce(new.metadata,'{}'::jsonb)||jsonb_build_object('food_subcategory_guard','v112','food_subcategory_guard_slug',v_slug,'food_subcategory_guard_at',now());
  return new;
end;
$function$;

revoke execute on function public.guard_safe_food_subcategory_v112() from public, anon, authenticated;

create trigger zzzzzz_safe_food_subcategory_guard_v112
before insert or update of name, brand, category_id, quantity_text, filter_group, metadata
on public.products
for each row execute function public.guard_safe_food_subcategory_v112();

drop function if exists public.guard_safe_food_subcategory_v111();