...
with src as (
  ...
  select ord,
         ...
         lead(btrim(line),1) over(order by ord) as next1,
         lead(btrim(line),2) over(order by ord) as next2,
         lead(btrim(line),3) over(order by ord) as next3,
         vf,vt,c
  from prepared,
  ...
  select ord,
         title,
         subtitle,
         replace(substring(next1 from '^([0-9][0-9 ]*) Kč$'),' ','')::numeric as price,
         substring(line from '^Výprodej -([0-9]+)%$')::int as discount_percent,
         replace(substring(next2 from '^([0-9][0-9 ]*) Kč$'),' ','')::numeric as old_price,
         vf,vt,c
  from product_lines
  where line ~ '^Výprodej -[0-9]+%$'
    and next1 ~ '^[0-9][0-9 ]* Kč$'
    and next2 ~ '^[0-9][0-9 ]* Kč$'
    and next3='Skladem'
), compact_cards0 as (
  select ord,
         title,
