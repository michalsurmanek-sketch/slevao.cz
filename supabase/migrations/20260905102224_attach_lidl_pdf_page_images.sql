create or replace function private.lidl_pdf_page_image_url(p_pdf_url text, p_page integer)
returns text
language sql
immutable strict
...
as $$
  select case
    ...
    when p_pdf_url !~* '^https://assets\.leaflets\.schwarz/leaflets/pdfs/.+\.pdf(?:\?.*)?$' then null
...
create or replace function private.attach_lidl_import_item_page_image()
returns trigger
language plpgsql
...
begin
  ...
  if coalesce(v_adapter,'') <> 'lidl-verified-pdf-text-v1' then
    return new;
  end if;

  v_page_url := private.lidl_pdf_page_image_url(v_pdf_url, new.source_page);
  ...
  new.raw_data := coalesce(new.raw_data,'{}'::jsonb) || jsonb_build_object(
    ...
    'page_image_source', 'lidl_official_schwarz_pdf_render',
...
drop trigger if exists attach_lidl_import_item_page_image_trg on public.leaflet_import_items;
create trigger attach_lidl_import_item_page_image_trg
before insert or update of source_page, import_id on public.leaflet_import_items
for each row execute function private.attach_lidl_import_item_page_image();

update public.leaflet_import_items lii
set raw_data = coalesce(lii.raw_data,'{}'::jsonb) || jsonb_build_object(
  'page_image_url', private.lidl_pdf_page_image_url(li.source_document_url, lii.source_page),
  'page_image_source', 'lidl_official_schwarz_pdf_render',
...
where lii.import_id = li.id
  ...
  and private.lidl_pdf_page_image_url(li.source_document_url, lii.source_page) is not null;
