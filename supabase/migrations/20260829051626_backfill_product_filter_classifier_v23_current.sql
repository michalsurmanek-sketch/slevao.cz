...
where coalesce(nullif(trim(p.filter_group),''),'other')='other'
  and lower(coalesce(p.metadata->>'source_store_slug','')) in ('kosik','globus','hruska','flop','norma','rohlik','kaufland')
  and public.infer_product_filter_group_v23(p.name,p.quantity_text,p.metadata) <> 'other'
  and p.id in (
