...
insert into private.automation_watchdog_store_targets(store_slug,recovery_action,cooldown,enabled,last_recovery_details,updated_at)
values ('dek','dek_products',interval '1 hour',true,'{}'::jsonb,now())
on conflict (store_slug) do update
set recovery_action=excluded.recovery_action,
