...
revoke all on function public.invoke_takko_products_sync() from public, anon, authenticated;

do $$
declare existing_job bigint;
begin
  select jobid into existing_job from cron.job where jobname='sync-takko-verified-products' limit 1;
  if existing_job is not null then perform cron.unschedule(existing_job); end if;
  perform cron.schedule('sync-takko-verified-products','0 6 * * *','select public.invoke_takko_products_sync();');
end;
$$;
