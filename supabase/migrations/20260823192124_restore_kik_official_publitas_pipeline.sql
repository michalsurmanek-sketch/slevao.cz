...
begin
  ...
  set is_active = (source_url='https://www.kik.cz/tvuj-online-letak'),
      ...
      next_review_at = case when source_url='https://www.kik.cz/tvuj-online-letak' then null else next_review_at end,
      last_error = case when source_url='https://www.kik.cz/tvuj-online-letak' then null else last_error end,
      updated_at = now()
  ...
  perform cron.schedule(
    'slevao-kik-source',
    '8,23,38,53 * * * *',
    $$select private.invoke_edge_function('sync-kik-source','{}'::jsonb,120000);$$
  );

  perform cron.schedule(
    'slevao-kik-products',
    '11,26,41,56 * * * *',
    $$select private.invoke_edge_function('sync-kik-products',jsonb_build_object('dry_run',false,'force',false),120000);$$
  );
end;
$$;
