create or replace function private.lidl_verified_rows_match_published_set(
  p_rows jsonb,
  p_store_id uuid,
  p_signature text,
  p_pdf_url text,
...
with expected as materialized (
  select
    ...
    trim(coalesce(x->>'title','')) as source_title,
...
begin
  ...
  where s.slug='lidl'
    ...
    and li.source_hash='lidl-verified-pdf-text-v1:'||(li.metadata->>'source_signature')
  ...
  if v_import_id is not null and coalesce(v_signature,'')<>'' then
    select count(*)::integer,
           coalesce(jsonb_agg(
             jsonb_build_object(
               'external_key',substring(o.external_id from length('lidlpdf:')+1),
               'title',lii.title,
             ...
             order by substring(o.external_id from length('lidlpdf:')+1),lii.title,lii.quantity_text,lii.price,o.unit_price
           ),'[]'::jsonb)
