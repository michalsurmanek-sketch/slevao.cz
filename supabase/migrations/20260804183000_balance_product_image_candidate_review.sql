...
begin
  ...
  product_ok := coalesce((review ->> 'product_matches')::boolean, false);
  ...
  then
    ...
    new.metadata := jsonb_set(
      ...
      '{review_tier}',
      to_jsonb(case
        when coalesce((review ->> 'clean_background')::boolean, false)
...
drop trigger if exists a_balance_product_image_candidate_review on public.product_image_candidates;
create trigger a_balance_product_image_candidate_review
before insert or update of status, metadata on public.product_image_candidates
for each row execute function public.balance_product_image_candidate_review();