...
begin
  ...
  if coalesce(v_stores && array['ca','cropp','house','reserved','sinsay','takko']::text[], false) then
    ...
    return query select 'mlecne-vyrobky','food',array['mlecne']::text[],0.97::numeric,'name-token-v2'; return;
        ...
        and v_text !~ ' rohlik cz ' then
    return query select 'pecivo','food',array['pecivo']::text[],0.97::numeric,'name-token-v2'; return;
  elsif v_text ~ ' (cokolada|bonbony|susenk|susenky|oplatky|pralinky|lentilky|karamelky|zvykacky) '
        or (v_text ~ ' tycinka ' and not v_is_drugstore and not v_is_pharmacy) then
    ...
    return query select 'auto','auto',array['auto']::text[],0.97::numeric,'name-token-v2'; return;
        ...
        and v_text !~ ' (chips|prichut|liker|rumovy|ovocne|chlebicky|pochoutkovy|susene|smoothie|dzus|juice|napoj|nektar) ' then
    return query select 'ovoce-zelenina','food',array['ovoce-zelenina']::text[],0.97::numeric,'name-token-v2'; return;
  elsif v_text ~ ' (mouka|cukr|ryze|testoviny|ocet|lusteniny|koreni) '
        and v_text !~ ' (pivo|lezak|radler) ' then
...
comment on function public.preview_product_taxonomy(uuid) is
  'Read-only taxonomy classifier preview v2 with retailer/context exclusions. Does not mutate products or offers; use for QA before backfill.';