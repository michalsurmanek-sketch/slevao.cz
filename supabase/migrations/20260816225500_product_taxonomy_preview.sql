...
begin
  select
    ' ' || trim(regexp_replace(lower(public.unaccent(coalesce(p.name,'') || ' ' || coalesce(p.brand,''))), '[^a-z0-9]+',' ','g')) || ' ',
  ...
  if coalesce(v_stores && array['ca','cropp','house','reserved','takko']::text[], false) then
  ...
  elsif ((v_text ~ ' (sampon|mydlo|deodorant|zubni|kartacek|praci|avivaz|cistic|toaletni|plenky|kosmetika|ustni voda) ') or v_text ~ ' sprchovy (gel|krem|olej) ') and v_text !~ ' stojanek na kartacek ' then
    ...
    return query select 'elektronika','electronics',array['elektronika']::text[],0.98::numeric,'name-token-v2'; return;
        ...
        and v_text !~ ' (ustni voda|micelarni voda|toaletni voda|parfemovana voda|antiperspirant|deodorant|hodiny|svicka|vune) ' then
    return query select 'napoje','drinks',array['napoje']::text[],0.97::numeric,'name-token-v6'; return;
  ...
  elsif v_text ~ ' (mleko|jogurt|eidam|gouda|emental|hermelin|niva|mozzarella|cheddar|cottage|maslo|smetana|tvaroh|kefir) '
        and v_text !~ ' (telove mleko|cistici mleko|opalovaci mleko|pletove mleko) ' then
    return query select 'mlecne-vyrobky','food',array['mlecne']::text[],0.97::numeric,'name-token-v6'; return;
    ...
    return query select 'auto','auto',array['auto']::text[],0.97::numeric,'name-token-v2'; return;
        ...
        and v_text !~ ' (chips|prichut|liker|rumovy|ovocne|chlebicky|pochoutkovy|susene|smoothie|dzus|juice|napoj|nektar|kasicka|kase|svacinka|prostredek|sedaci souprava|pohovka|nabytek|rybi salat|majonez|gummies|vitamin) ' then
    return query select 'ovoce-zelenina','food',array['ovoce-zelenina']::text[],0.97::numeric,'name-token-v6'; return;
...
comment on function public.preview_product_taxonomy(uuid) is 'Read-only taxonomy classifier preview v6 with retailer/context exclusions. Does not mutate products or offers; use for QA before backfill.';