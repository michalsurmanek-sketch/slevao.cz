...
begin
  ...
  -- Rychlé filtry popisují, co výrobek je, ne pouze ingredienci v názvu.
  -- Pečivo se sýrem/máslem/mlékem/vejcem proto zůstává pečivem.
  if bread then
    milk := false;
    eggs := false;
    butter := false;
  ...
  prsa_meat := s ~ '\mprsa\M'
    and s !~ '\m(paska|podprsenk[a-z0-9]*|nalepk[a-z0-9]*|ochran[a-z0-9]*|bradavk[a-z0-9]*|kosmetik[a-z0-9]*|odev[a-z0-9]*|trick[a-z0-9]*|top|plavk[a-z0-9]*)\M';

  pet_context := s ~ '\m(akinu|dingo|dog snaq|prevital|vetamix|vitakraft|shinycat|huhubamboo|propesko|felix fantastic|whiskas|pedigree|purina)\M'
    or s ~ '\m(pro psy|pro kocky|pro psa|granule|pamlsk[a-z0-9]*|krmivo|kapsicky pro)\M';
  meat_alternative := s ~ '\m(alternativ[a-z0-9]*|vegetari[a-z0-9]*|vegansk[a-z0-9]*|rostlinn[a-z0-9]*|veggie)\M'
    and s ~ '\mmasov[a-z0-9]*\M';

  meat := s ~ '\m(maso|masov[a-z0-9]*|vepr[a-z0-9]*|hovez[a-z0-9]*|kure[a-z0-9]*|kurec[a-z0-9]*|kruti|kruta|kruty|kachn[a-z0-9]*|jehnec[a-z0-9]*|kralic[a-z0-9]*|krkovic[a-z0-9]*|kyta|plec|kotlet[a-z0-9]*|panenk[a-z0-9]*|svickov[a-z0-9]*|rosten[a-z0-9]*|steak[a-z0-9]*|zebro|zeber|bucek|koleno|gulasov[a-z0-9]*|jatr[a-z0-9]*)\M'
    or prsa_meat;
  if pet_context or meat_alternative then meat := false; end if;

  chicken := s ~ '\m(kure[a-z0-9]*|kurec[a-z0-9]*)\M'
    or prsa_meat;
  if pet_context or meat_alternative then chicken := false; end if;
  ...
  veg := s ~ '\m(brambor[a-z0-9]*|cibul[a-z0-9]*|cesnek[a-z0-9]*|rajcat[a-z0-9]*|paprik[a-z0-9]*|okurk[a-z0-9]*|mrkev|petrzel|celer[a-z0-9]*|kedlub[a-z0-9]*|kvetak[a-z0-9]*|brokolic[a-z0-9]*|cuket[a-z0-9]*|lilek|redkv[a-z0-9]*|repa|kapust[a-z0-9]*|zeli|salat[a-z0-9]*|spenat[a-z0-9]*|hras[a-z0-9]*|kukuric[a-z0-9]*|fazol[a-z0-9]*)\M'
    and s !~ '\m(napoj[a-z0-9]*|dzus[a-z0-9]*|stava|smoothie|pomazank[a-z0-9]*|protlak[a-z0-9]*|pyre|omack[a-z0-9]*|chips[a-z0-9]*|syrov[a-z0-9]*|syr[a-z0-9]*|tunak[a-z0-9]*|lahudk[a-z0-9]*|hotov[a-z0-9]*)\M';
  ...
  if s ~ '\meidam[a-z0-9]*\M' and not bread then tags:=array_append(tags,'eidam'); end if;
  if s ~ '\mgouda\M' and not bread then tags:=array_append(tags,'gouda'); end if;
