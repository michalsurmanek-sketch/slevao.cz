...
begin
  ...
  -- An authenticated INSERT is always an atomic completion of a concrete live
  -- shopping list. Keep shopping_list_id nullable for historical rows because
  -- the FK intentionally uses ON DELETE SET NULL after the source list is gone.
  if tg_op = 'INSERT'
  ...
  -- Shopping Insights is explicitly a seven-day planned estimate. Validate the
  -- same bounded window here: an offer must overlap [purchase_date, +7 days],
  -- while still matching product and stored price exactly enough for history.
  if exists (
    select 1
    from jsonb_array_elements(new.items) as item(value)
    ...
    raise exception 'Historie obsahuje nabídku mimo povolený sedmidenní odhad nebo s nesouhlasící cenou.';
  end if;
