-- Keep one logical price alert per user/product/store and merge repeated inserts safely.
...
begin
  ...
  set target_price = new.target_price,
      ...
      last_triggered_at = case
        when pa.target_price is distinct from new.target_price
          or pa.is_active is distinct from coalesce(new.is_active, true)
        then null
        else pa.last_triggered_at
      end,
      updated_at = now()
...
create unique index if not exists price_alerts_one_logical_alert_uidx
on public.price_alerts (user_id, product_id, store_id) nulls not distinct
