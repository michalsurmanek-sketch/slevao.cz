...
begin
  ...
  for candidate in
    ...
    where l.status in ('review','published')
      and (coalesce(l.metadata->>'content_type','') like 'image/%'
      ...
      and (
        coalesce(l.metadata->>'crop_status','') not in ('completed','running')
        or (
          l.metadata->>'crop_status'='running'
          and coalesce((l.metadata->>'crop_queued_at')::timestamptz,'epoch'::timestamptz) < now()-interval '30 minutes'
        )
        or (
          l.metadata->>'crop_status'='failed'
          and coalesce((l.metadata->>'crop_finished_at')::timestamptz,'epoch'::timestamptz) < now()-interval '6 hours'
        )
      )
    order by
  ...
  loop
    ...
    set metadata=jsonb_set(
      jsonb_set(coalesce(metadata,'{}'::jsonb),'{crop_status}','"running"'::jsonb,true),
