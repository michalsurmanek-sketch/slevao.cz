...
begin
  ...
  review_reason := lower(coalesce(review ->> 'reason', ''));
  ...
  effective_match := product_match
    or (
      package_match is true
      and price_overlay
      and review_reason not like '%jiný produkt%'
      and review_reason not like '%jiny produkt%'
      and review_reason not like '%jiná značka%'
      and review_reason not like '%jina znacka%'
      and review_reason not like '%neodpovídá požadovanému produktu%'
      and review_reason not like '%neodpovida pozadovanemu produktu%'
    );
