...
begin
  ...
  if not found then
    insert into public.leaflet_imports (
      ...
      v_source.source_id,
      v_source.store_id,
      v_source.source_document_url,
      v_hash,
      'processing',
      coalesce(v_source.coverage_scope, 'national'),
      v_source.region_code,
      v_source.city_name,
      v_source.store_location_name,
      jsonb_build_object(
    ...
    v_request_id := private.invoke_edge_function(
      ...
      jsonb_build_object('import_id', v_verified.id),
      120000
    );
  ...
  if v_verified.status = 'failed' then
    return jsonb_build_object(
      ...
      'stage', 'failed',
      'import_id', v_verified.id,
      'error', v_verified.error_message
    );
  end if;
  ...
  if not v_has_extraction then
    if v_verified.status not in ('processing', 'queued')
      ...
      return jsonb_build_object('ok', true, 'stage', 'extracting_retry', 'import_id', v_verified.id, 'request_id', v_request_id);
    end if;

    return jsonb_build_object('ok', true, 'stage', 'waiting_for_extraction', 'import_id', v_verified.id);
  end if;

  if v_parser <> 'billa-coordinate-v1' then
    update public.leaflet_imports
    ...
    return jsonb_build_object('ok', true, 'stage', 'parsing', 'import_id', v_verified.id, 'request_id', v_request_id);
  end if;
  ...
  if v_verified.status = 'published' then
    ...
    return jsonb_build_object('ok', true, 'stage', 'published', 'import_id', v_verified.id, 'published_count', v_verified.product_count);
  end if;
  ...
  if v_verified.status = 'review' and v_approved > 0 then
    ...
    return jsonb_build_object('ok', true, 'stage', 'publishing', 'import_id', v_verified.id, 'approved', v_approved, 'request_id', v_request_id);
  end if;
  ...
  return jsonb_build_object(
    ...
    'stage', 'waiting',
    'import_id', v_verified.id,
    'status', v_verified.status,
    'approved', v_approved
  );
end;
$$;

revoke all on function public.reconcile_billa_verified_pipeline() from public, anon, authenticated;
grant execute on function public.reconcile_billa_verified_pipeline() to service_role;

comment on function public.reconcile_billa_verified_pipeline() is
  'State-machine pipeline that converts the newest generic BILLA PDF discovery into a coordinate-verified 0.99 offer set.';
...
begin
  ...
  from cron.job
  where jobname = 'reconcile-billa-verified-products'
  limit 1;
  ...
  perform cron.schedule(
    'reconcile-billa-verified-products',
    '*/5 * * * *',
    'select public.reconcile_billa_verified_pipeline();'
  );
end;
$$;
