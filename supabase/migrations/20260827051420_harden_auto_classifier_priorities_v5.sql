...
declare
  ...
  v_auto_quota integer := greatest(1,v_limit/3);
  v_auto_count integer := 0;
  v_unresolved_limit integer := v_limit;
  v_checked integer := 0;
  v_changed integer := 0;
begin
  ...
  v_unresolved_limit := greatest(v_limit-v_auto_count,0);

  with auto_old as materialized (
    ...
    where p.is_active=true
      ...
      and not exists(select 1 from auto_old a where a.id=p.id)
    order by p.updated_at desc nulls last,p.id desc
    limit v_unresolved_limit
    for update skip locked
  ...
  return jsonb_build_object(
    ...
    'auto_recheck_quota',v_auto_quota
  );
end;
$$;
