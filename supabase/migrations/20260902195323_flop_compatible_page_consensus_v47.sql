create or replace function private.refresh_flop_compatible_page_consensus_v47()
returns jsonb
language plpgsql
security definer
...
begin
  with current_flop as (
    ...
    join public.stores st on st.id=o.store_id and st.slug='flop'
    ...
    select distinct p.id,cf.import_id,cf.leaflet_page,ps.consensus_group,ps.known_count
    from current_flop cf
    join page_stats ps using(import_id,leaflet_page)
    join public.products p on p.id=cf.product_id
  ...
  set metadata=coalesce(p.metadata,'{}'::jsonb) || jsonb_build_object(
      ...
      'source_page_consensus_compatibility','food-mass_or_drinks-volume-v47',
...
revoke all on function private.refresh_flop_compatible_page_consensus_v47() from public, anon, authenticated;
grant execute on function private.refresh_flop_compatible_page_consensus_v47() to postgres, service_role;
...
as $function$ select 47 $function$;
...
begin
  ...
  if coalesce(nullif(trim(new.filter_group),''),'other')='other' or v_old_auto or v_new_auto then
    v_inferred := 'other';
    if lower(trim(coalesce(v_source_store,''))) in ('lidl','hruska','flop')
    ...
    if v_inferred <> 'other' then
      ...
      if v_page_consensus then
        new.classification_source := case when lower(trim(coalesce(v_source_store,'')))='flop' then 'source-page-consensus-v47' else 'source-page-consensus-v43' end;
...
begin
  for v_jobid in select jobid from cron.job where jobname='classify-flop-page-consensus' loop
    perform cron.unschedule(v_jobid);
  end loop;
  perform cron.schedule('classify-flop-page-consensus','19 * * * *','select private.refresh_flop_compatible_page_consensus_v47();');
end;
$block$;

select private.refresh_flop_compatible_page_consensus_v47();