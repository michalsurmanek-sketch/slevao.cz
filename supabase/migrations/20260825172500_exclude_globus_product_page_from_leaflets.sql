-- Globus product API imports use /hypermarket/akcni-nabidka but are not browseable leaflet documents.
-- Only the dedicated /letaky/ document source (or its explicit historical adapter) may use the Globus resolver.
  ...
  where li.status = 'published'
    ...
    and (
      ...
      or (
        s.slug = 'globus'
        and (
          coalesce(li.metadata->>'adapter', '') = 'store:globus-html'
          or coalesce(li.source_document_url, '') like 'https://%globus.cz/%/letaky/%'
        )
      )
      or (
        s.slug = 'teta'
