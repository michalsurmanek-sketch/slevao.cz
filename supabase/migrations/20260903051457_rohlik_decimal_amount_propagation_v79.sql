...
begin
  ...
  v_raw:=lower(v_amount);
  if v_raw !~ '^(cca[[:space:]]+)?[0-9]+([,.][0-9]+)?[[:space:]]*(g|kg|ml|l|ks)$' then return new; end if;

  update public.products p
     set quantity_text=v_amount,
         metadata=coalesce(p.metadata,'{}'::jsonb) || jsonb_build_object(
           'source_quantity_source','rohlik-offer-amount-v79',
...
with current_rohlik as (
  ...
  where o.status='published' and o.is_verified=true
    ...
    and lower(o.metadata->>'amount') ~ '^(cca[[:space:]]+)?[0-9]+([,.][0-9]+)?[[:space:]]*(g|kg|ml|l|ks)$'
