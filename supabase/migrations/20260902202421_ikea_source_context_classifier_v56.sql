create or replace function public.infer_product_filter_group_ikea_context_v56(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(sada taliru|osuska|ram .*postel|vlozka .*zasuv|vaza|nasten.*hodin|matrace|kryci podlozka)' then return 'home'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 56 $function$;
...
where p.is_active is true
  ...
  and lower(trim(coalesce(p.metadata->>'source_store_slug','')))='ikea'
  and exists (
    select 1 from public.offers o
    join public.stores s on s.id=o.store_id and s.slug='ikea'
