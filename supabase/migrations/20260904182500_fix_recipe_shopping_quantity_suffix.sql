-- Keep the recipe display label (for example "Hovězí maso (800 g)") intact,
-- but strip a trailing recipe quantity suffix before offer matching.
-- This mirrors the production definition of public.get_public_shopping_list_candidates.

CREATE OR REPLACE FUNCTION public.get_public_shopping_list_candidates(p_queries text[], p_limit_per_query integer DEFAULT 30)
 RETURNS TABLE(query_text text, query_key text, candidate_rank integer, offer jsonb, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  for rec in
    ...
    from (
      select btrim(value) as query_text,
        coalesce(nullif(btrim(regexp_replace(btrim(value), '[[:space:]]*\([[:space:]]*[0-9]+([.,][0-9]+)?[[:space:]]+(kg|g|ml|l|ks|balení|stroužek|stroužky|stroužků)[[:space:]]*\)[[:space:]]*$', '', 'i')), ''), btrim(value)) as base_text,
