-- Keep the public leaflet catalogue limited to documents the leaflet viewer can actually render.
-- Product/sale HTML imports remain published offers, but they must not masquerade as browseable leaflets.
...
with params as (
  ...
  where li.status = 'published'
    ...
    and (
      ...
      -- These two HTML sources have strict, host-allowlisted resolvers in store-leaflet-document.
      or (
        s.slug = 'globus'
        and coalesce(li.source_document_url, '') like 'https://%globus.cz/%'
      )
      or (
        s.slug = 'teta'
        and (
