...
parallel safe
set search_path to ''
as $function$ select 19; $function$;

update public.products
set name=name
where coalesce(nullif(trim(filter_group),''),'other')='other'
  and (
    (metadata->>'kaufland_category'='Ovoce, zelenina, rostliny'
      and public.normalize_text(concat_ws(' ',coalesce(name,''),coalesce(quantity_text,''))) ~ '(^| )(chryzantema|dracinec|kytice|orchidea|ruze|sachor|cyperus|zamioculcas|mucenka|passionfruit)( |$)')
    or (metadata->>'kaufland_category'='Drogerie, dětská výživa a péče, krmiva'
      and public.normalize_text(concat_ws(' ',coalesce(name,''),coalesce(quantity_text,''))) ~ '(^| )(prebalovaci podlozk[a-z0-9]*|kosmeticke ubrousk[a-z0-9]*|kondom[a-z0-9]*|nahradni hlavic[a-z0-9]*|nahradni brit[a-z0-9]*)( |$)')
    or (metadata->>'kaufland_category'='Dům, domácnost'
      and public.normalize_text(concat_ws(' ',coalesce(name,''),coalesce(quantity_text,''))) ~ '(^| )stropni led svetlo( |$)')
    or metadata->>'kaufland_category' in ('Mléčné výrobky, tuky, vejce','Mražené','Základní potraviny, pečivo','Maso, drůbež, uzeniny')
  );
