...
begin
  ...
  -- v20: strong grocery identities before the generic public fallback (which treats "pohar" as homeware).
  if n ~ '(^| )mullermilch( |$)' then return 'drinks'; end if;
  if n ~ '(^| )(dresink[a-z0-9]*|salatov[a-z0-9]* zalivk[a-z0-9]*|chilli olej|korenici smes|studentsky mix|papri chup|mlekarna kunin|susene ovocne rolovane platky|kinder cokoladova vajicka|opavia zlate venecky)( |$)'
  ...
  if n ~ '(^| )(smes do ostrikovacu|kanystr na pohonne hmoty|modulator do auta)( |$)' then return 'auto'; end if;
  ...
  if n ~ '(^| )(doza plastov[a-z0-9]*|sada doz plastov[a-z0-9]*|cajove svicky|metla na slehani|naberacka|obracecka|sada mis|plech na peceni|napenovac mleka)( |$)' then return 'home'; end if;
  ...
  -- v19: late Kaufland metadata fallback. Strong name/semantic signals above always win.
...
-- Re-run only high-confidence v20 candidates. Explicit/manual groups remain untouched.
...
and (
  public.normalize_text(concat_ws(' ',coalesce(name,''),coalesce(quantity_text,''))) ~ '(^| )(bebe friends|mikrotuzk[a-z0-9]*|popisovac[a-z0-9]*|propisk[a-z0-9]*|nekonecna tuzk[a-z0-9]*|fineliner|suche pastel[a-z0-9]*|modelovaci hmota|skicak|frixion|stabilo|desky na sesity|kancelarsky papir|sanon|laminovaci folie|poznamkove listeck[a-z0-9]*|plastove pravitko|mini zvyraznovac[a-z0-9]*|mullermilch|dresink[a-z0-9]*|salatov[a-z0-9]* zalivk[a-z0-9]*|chilli olej|korenici smes|studentsky mix|papri chup|mlekarna kunin|susene ovocne rolovane platky|kinder cokoladova vajicka|opavia zlate venecky|pistacie|skrabadlo|podlozka na krmeni|skrabacich rohozek|smes do ostrikovacu|kanystr na pohonne hmoty|modulator do auta|doza plastov[a-z0-9]*|sada doz plastov[a-z0-9]*|cajove svicky|metla na slehani|naberacka|obracecka|sada mis|plech na peceni|napenovac mleka|chytre hodinky|powerbanka|skartovack[a-z0-9]*)( |$)'
  or (
    public.normalize_text(concat_ws(' ',coalesce(name,''),coalesce(quantity_text,''))) ~ '(^| )orezavatk[a-z0-9]*( |$)'
    and public.normalize_text(concat_ws(' ',coalesce(name,''),coalesce(quantity_text,''))) !~ '(^| )(wet n wild|kosmet[a-z0-9]*|elektrick[a-z0-9]*)( |$)'
  )
);