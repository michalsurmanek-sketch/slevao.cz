...
begin
  ...
  if v_import.id is not null and v_offer_count>0 then
    update public.store_product_sync_state
       set health_status='ok',
           ...
           last_run_at=now(),
           last_success_at=coalesce(v_import.updated_at,last_success_at,now()),
  ...
  return jsonb_build_object('ok',true,'date',v_today,'offers',v_offer_count,'import_id',v_import.id,'valid_from',v_import.detected_valid_from,'valid_to',v_import.detected_valid_to);
end;
$function$;
