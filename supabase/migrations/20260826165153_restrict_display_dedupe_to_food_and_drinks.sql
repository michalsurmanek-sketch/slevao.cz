...
with joined as materialized (
  ...
  select p.*,
         row_number() over(
           partition by
             case when p.effective_filter_group in ('food','drinks') then p.store_slug else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then p.display_title_key else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then p.display_brand_key else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then p.display_quantity_key else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then p.price::text else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then p.valid_from::text else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then p.valid_to::text else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then coalesce(p.coverage_scope,'national') else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then coalesce(p.region_code,'') else p.offer_id::text end,
             case when p.effective_filter_group in ('food','drinks') then coalesce(p.city_name,'') else p.offer_id::text end,
             case
               when p.effective_filter_group not in ('food','drinks') then p.offer_id::text
               when coalesce(p.coverage_scope,'national')='national' then ''
               else coalesce(p.store_location_name,'')
             end
