create or replace function public.infer_product_filter_group_generic_terms_v48(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(pomazankove|aspikovy dortik|rostlinny tuk|(^| )pudding( |$))' then return 'food'; end if;
  if n ~ '(^| )eiskaffee( |$)|svetle vycepni' then return 'drinks'; end if;
  return public.infer_product_filter_group_generic_terms_v45(p_name,p_quantity_text);
end;
$function$;
