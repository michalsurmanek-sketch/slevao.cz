create or replace function public.infer_product_filter_group_benu_context_v55(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '((^| )dog( |$).*pipet|pipet.*(^| )dog( |$)|pro psy|pro kocky)' then return 'pets'; end if;
  if n ~ '(ocni kapky|peroralni kapky|leciva naplast|gel proti bolesti|pastilk|(^| )tbl([ .0-9]|$)|neobalene tablety|bylinny extrakt|orm spr sol)' then return 'pharmacy'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 55 $function$;
...
create or replace function public.propagate_benu_product_context_v55()
returns trigger
language plpgsql
set search_path to 'public','pg_temp'
as $function$
declare
  v_is_benu boolean := false;
begin
  if new.product_id is null or new.store_id is null then return new; end if;
  select exists(select 1 from public.stores s where s.id=new.store_id and s.slug='benu') into v_is_benu;
  if v_is_benu then
    update public.products p
       set metadata=coalesce(p.metadata,'{}'::jsonb) || jsonb_build_object(
         'source_benu_context',true,
         'source_benu_context_source','offer-store-context-v55'
       )
     where p.id=new.product_id
       and coalesce(p.metadata->>'source_benu_context','false')<>'true';
  end if;
  return new;
end;
$function$;

drop trigger if exists trg_propagate_benu_product_context_v55 on public.offers;
create trigger trg_propagate_benu_product_context_v55
after insert or update of store_id,product_id on public.offers
for each row execute function public.propagate_benu_product_context_v55();
...
where exists (
  ...
  join public.stores s on s.id=o.store_id and s.slug='benu'
