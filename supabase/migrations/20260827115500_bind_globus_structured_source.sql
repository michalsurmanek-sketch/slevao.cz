-- Globus has multiple active leaflet sources. The generic structured publisher
-- currently selects the most recently successful source, which can be the document
-- leaflet instead of the API source. Bind this narrow wrapper to the exact source
-- URL and verify the resulting import before committing the batch.
...
begin
  ...
  select id into v_expected_source_id
  from public.leaflet_sources
  where store_id = v_store_id
    and is_active = true
    and source_url = p_source_document_url
  limit 1;
  if v_expected_source_id is null then
  ...
  v_filtered_count := jsonb_array_length(v_filtered_rows);
  ...
  -- The generic publisher currently chooses the active source with the newest
  -- last_success_at. Promote the exact API source inside this transaction only;
  -- any later failure rolls this change back atomically.
  ...
  if v_source_id is distinct from v_expected_source_id then
    raise exception 'Globus publisher used unexpected source % instead of %.', v_source_id, v_expected_source_id;
  end if;
  ...
  update public.store_product_sync_state
     set adapter_name = 'globus-action-products-api-v1',
         ...
         health_reason = format(
           'Globus Olomouc: publikováno %s oficiálních API nabídek; API reportuje %s; pod 2 Kč vynecháno %s.',
           v_scoped,
  ...
  return v_result || jsonb_build_object(
    ...
    'store_location_name', 'Globus Olomouc',
    'house_number', 4008,
    'source_id', v_expected_source_id,
