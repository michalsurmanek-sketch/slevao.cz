...
with offer_stats as (
  ...
  select p.id,p.name,p.brand,p.ean,p.quantity_text,p.image_checked_at,
    case
    ...
    end as priority_image_checked_at
...
select p.id,p.name,p.brand,p.ean,p.quantity_text,s.active_offer_count,s.active_store_count,s.last_offer_at,p.priority_image_checked_at as image_checked_at
from offer_stats s join eligible_products p on p.id=s.product_id
where s.active_offer_count > 0
union all
select p.id,p.name,p.brand,p.ean,p.quantity_text,0::bigint,0::bigint,s.last_offer_at,p.priority_image_checked_at
from eligible_products p left join offer_stats s on s.product_id=p.id
where coalesce(s.active_offer_count,0)=0;
...
drop view if exists public.products_missing_verified_images_active;
