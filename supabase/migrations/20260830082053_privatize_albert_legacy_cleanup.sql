create or replace function private.cleanup_stale_albert_legacy_runs(p_age interval default interval '90 minutes')
returns integer
language plpgsql
security definer
...
begin
  ...
  perform cron.schedule('cleanup-stale-albert-legacy-runs','37 3 * * *',$cron$select private.cleanup_stale_albert_legacy_runs();$cron$);
end $$;

drop function if exists public.cleanup_stale_albert_legacy_runs(interval);