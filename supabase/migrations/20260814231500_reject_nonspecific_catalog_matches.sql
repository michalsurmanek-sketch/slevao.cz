-- Generic labels must not resolve to a concrete catalogue product without an EAN.
...
begin
  ...
  if normalized_title = '' or not public.product_label_is_specific(p_title) then return; end if;
