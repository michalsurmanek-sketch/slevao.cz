create or replace function public.infer_product_filter_group_rohlik_context_v61(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(piadina|plack|broskev|rukola)' then return 'food'; end if;
  if n ~ '(^| )(apa|ipa)( |$)' or r ~ '[0-9]{1,2}°.*plech' then return 'drinks'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 61 $function$;
...
where p.is_active is true
  ...
  and lower(trim(coalesce(p.metadata->>'source_store_slug','')))='rohlik'
  and exists (
    select 1 from public.offers o
    join public.stores s on s.id=o.store_id and s.slug='rohlik'
