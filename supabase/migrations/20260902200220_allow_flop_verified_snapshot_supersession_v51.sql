...
begin
  if old.status='published'
    ...
    if old.store_id=v_flop_store_id then
      begin
        v_superseding_import:=nullif(new.metadata->>'flop_superseded_by_import_id','')::uuid;
      exception when others then
        v_superseding_import:=null;
      end;

      if v_superseding_import is not null
         and exists (
           select 1
           from public.leaflet_imports li
           where li.id=v_superseding_import
             and li.store_id=old.store_id
             ...
             and li.detected_valid_from=old.valid_from
             and li.detected_valid_to=old.valid_to
         ) then
        return new;
      end if;
...
begin
  ...
  select count(*)::integer,
         count(distinct lii.product_id)::integer
    into v_candidate_count,v_distinct_products
  ...
  update public.offers o
     set metadata=coalesce(o.metadata,'{}'::jsonb) || jsonb_build_object(
           'flop_superseded_by_import_id',p_import_id,
           'flop_superseded_at',v_now
         ),
         status='expired',
         updated_at=v_now
   where o.store_id=v_store_id
     and o.status='published'
     and coalesce(o.store_location_name,'')='FLOP TOP'
     and o.valid_from=v_import.detected_valid_from
     and o.valid_to=v_import.detected_valid_to
