alter function public.publish_albert_publitas_text_offers_v4_strong(text,jsonb)
  set schema private;

alter function private.publish_albert_publitas_text_offers_v4_strong(text,jsonb)
  rename to publish_albert_publitas_text_offers_v4_strong_full;

revoke all on function private.publish_albert_publitas_text_offers_v4_strong_full(text,jsonb)
  from public, anon, authenticated, service_role;
grant execute on function private.publish_albert_publitas_text_offers_v4_strong_full(text,jsonb)
  to postgres;

revoke all on function public.publish_albert_publitas_text_offers_v4(text,jsonb)
  from public, anon, authenticated, service_role;
grant execute on function public.publish_albert_publitas_text_offers_v4(text,jsonb)
  to postgres;

create function public.publish_albert_publitas_text_offers_v4_strong(
  p_signature text,
  p_rows jsonb
)
returns jsonb
...
begin
  ...
  if v_count < 50 then
    raise exception 'Albert strong-identity sada po odstranění neúplných variant obsahuje jen % nabídek; bezpečnostní minimum je 50.',v_count;
  end if;
...
revoke all on function public.publish_albert_publitas_text_offers_v4_strong(text,jsonb)
  from public,anon,authenticated;
grant execute on function public.publish_albert_publitas_text_offers_v4_strong(text,jsonb)
