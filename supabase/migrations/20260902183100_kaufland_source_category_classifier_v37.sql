...
set search_path to 'public','pg_temp'
as $function$ select 37 $function$;
...
declare
  ...
  v_source_path text;
begin
  if coalesce(nullif(trim(new.metadata->>'source_store_slug'),''),'')=''
     and coalesce(new.metadata->>'created_from_kaufland_ssr','false')='true'
     and nullif(trim(new.metadata->>'kaufland_category'),'') is not null then
  ...
  if coalesce(nullif(trim(new.filter_group),''),'other')='other' or v_old_auto or v_new_auto then
    ...
    if v_inferred <> 'other' then
      ...
      if v_source_category then new.classification_source := 'source-category-v37';
