-- Track the already-deployed v19 Kaufland metadata fallback in migration history.
-- This is intentionally replay-safe and preserves the current classifier behavior.
...
-- Re-run only the narrow Kaufland metadata candidates. Already-correct rows remain stable.
update public.products
set name=name
where coalesce(metadata->>'kaufland_category','') in (
  'Ovoce, zelenina, rostliny',
  'Drogerie, dětská výživa a péče, krmiva',
  'Dům, domácnost',
  'Mléčné výrobky, tuky, vejce',
  'Mražené',
  'Základní potraviny, pečivo',
  'Maso, drůbež, uzeniny'
)
and (
