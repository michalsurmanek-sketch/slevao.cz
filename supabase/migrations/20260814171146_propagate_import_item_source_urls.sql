...
drop trigger if exists trg_propagate_import_item_source_to_offer on public.leaflet_import_items;
create trigger trg_propagate_import_item_source_to_offer
after insert or update on public.leaflet_import_items
for each row
when (new.status in ('published','ignored'))
execute function private.propagate_import_item_source_to_offer();
