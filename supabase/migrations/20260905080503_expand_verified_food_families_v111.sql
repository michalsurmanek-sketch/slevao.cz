drop trigger if exists zzzzzz_safe_food_subcategory_guard_v110 on public.products;

create or replace function public.guard_safe_food_subcategory_v111()
returns trigger
language plpgsql
security invoker
...
begin
  ...
  new.classification_source:='food-subcategory-guard-v111';
  new.classified_at:=now();
  new.metadata:=coalesce(new.metadata,'{}'::jsonb)||jsonb_build_object('food_subcategory_guard','v111','food_subcategory_guard_slug',v_slug,'food_subcategory_guard_at',now());
  return new;
end;
$function$;

revoke execute on function public.guard_safe_food_subcategory_v111() from public, anon, authenticated;

create trigger zzzzzz_safe_food_subcategory_guard_v111
before insert or update of name, brand, category_id, quantity_text, filter_group, metadata
on public.products
for each row execute function public.guard_safe_food_subcategory_v111();

drop function if exists public.guard_safe_food_subcategory_v110();