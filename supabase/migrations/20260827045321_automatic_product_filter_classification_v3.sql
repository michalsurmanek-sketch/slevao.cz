...
begin
  ...
  if n ~ '(^| )(krmivo|granule|stelivo|pro psy|pro psa|pro kocky|pamlsk[a-z0-9]*|kapsicky pro psy|kapsicky pro kocky|konzerva pro psy|konzerva pro kocky|whiskas|pedigree|purina|vetamix)( |$)' then return 'pets'; end if;
  ...
  if n ~ '(^| )(ostiepok[a-z0-9]*|poloostiepok[a-z0-9]*)( |$)' then return 'food'; end if;

  v_group := public.infer_public_filter_group(p_name,null);
  if v_group <> 'other' then
    if v_group='garden' and n ~ '(^| )(syr[a-z0-9]*|halloumi|hermelin[a-z0-9]*|camembert[a-z0-9]*|ostiepok[a-z0-9]*|poloostiepok[a-z0-9]*|klobas[a-z0-9]*|parek|parky|maso|kure[a-z0-9]*|vepr[a-z0-9]*|hovez[a-z0-9]*)( |$)' then return 'food'; end if;
    return v_group;
  end if;

  if n ~ '(^| )(hummus|kreveta|krevety|chobotnice|makronka|makronky|babovka|jerky|jiska|sushi|medovnik|krajic|chilli con carne|salamky)( |$)' then return 'food'; end if;
  if n ~ '(^| )(merlot|cabernet|sauvignon|veltliner|cerveza|tonic|kafe|coffee|secco|elixir)( |$)' then return 'drinks'; end if;
