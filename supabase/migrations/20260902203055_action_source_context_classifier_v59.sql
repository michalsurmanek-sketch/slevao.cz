create or replace function public.infer_product_filter_group_action_context_v59(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(flisovy pled|koberec|obedovy box)' then return 'home'; end if;
  if n ~ 'zapisnik' then return 'school'; end if;
  if n ~ 'sliz' then return 'toys'; end if;
  if n ~ 'vonny stromecek' then return 'auto'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 59 $function$;
...
where p.is_active is true
  ...
  and lower(trim(coalesce(p.metadata->>'source_store_slug','')))='action'
  and exists (
    select 1 from public.offers o
    join public.stores s on s.id=o.store_id and s.slug='action'
