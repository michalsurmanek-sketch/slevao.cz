create or replace function public.infer_product_filter_group_makro_context_v57(p_name text,p_quantity_text text)
returns text
language plpgsql
...
declare
  ...
  r text := lower(unaccent(coalesce(p_name,'')));
begin
  if n ~ 'frizzante' or r ~ '[0-9]+([,.][0-9]+)?\s*%.*[0-9]+([,.][0-9]+)?\s*(ml|l)(\s|$)' then return 'drinks'; end if;
  if n ~ '(brusink|mraz|hras|tycink|tycine|mlecny rez|olej repkov|minonk|zvykack)' then return 'food'; end if;
  if n ~ 'kapesnik' then return 'drugstore'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 57 $function$;
...
where p.is_active is true
  ...
  and lower(trim(coalesce(p.metadata->>'source_store_slug','')))='makro'
  and exists (
    select 1 from public.offers o
    join public.stores s on s.id=o.store_id and s.slug='makro'
