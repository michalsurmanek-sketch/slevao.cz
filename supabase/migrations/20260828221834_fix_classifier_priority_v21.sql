...
begin
  ...
  -- v21: BEBE Friends is a stationery line; do not let the generic cosmetic "korektor" rule win.
  if n ~ '(^| )bebe friends( |$)' then
    return 'school';
  end if;
...
parallel safe
set search_path to ''
as $function$ select 21; $function$;

-- Re-evaluate only the priority-fix product family and the missed v20 home candidate.
...
and (
  public.normalize_text(coalesce(name,'')) ~ '(^| )bebe friends( |$)'
  or public.normalize_text(coalesce(name,'')) ~ '(^| )cajove svicky( |$)'
);