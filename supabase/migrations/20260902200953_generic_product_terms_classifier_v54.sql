create or replace function public.infer_product_filter_group_generic_terms_v54(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(oblicejovy olej|hydratacni pece|nalepky na nehty)' then return 'drugstore'; end if;
  if n ~ '(^| )(dekorace|svicen)( |$)|prouteny venec' then return 'home'; end if;
  if n ~ '(^| )zabky( |$)' then return 'fashion'; end if;
  return public.infer_product_filter_group_generic_terms_v53(p_name,p_quantity_text);
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 54 $function$;
