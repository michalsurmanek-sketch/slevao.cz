create or replace function public.slevao_products_curated_equivalent(p_product_a uuid, p_product_b uuid)
returns boolean
language sql
stable
security definer
     ...
     and (
       ...
       or exists (
         ...
         where e.is_active = true
           and e.match_method = 'curated_brand_quantity'
           and e.confidence = 1
...
begin
  if new.is_active = true and new.product_id is not null then
    ...
    from public.offers o
    where public.slevao_products_curated_equivalent(new.product_id, o.product_id)
      and o.status = 'published'
      and o.is_verified = true
      and o.valid_to >= v_today
      and o.valid_from <= v_today + 7
...
revoke all on function public.slevao_products_curated_equivalent(uuid,uuid) from public, anon, authenticated;
