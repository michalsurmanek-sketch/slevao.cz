create or replace function public.infer_product_filter_group_auto_kelly_context_v63(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  ...
  if n ~ '(ochranny nastrik|opravu plast)' then return 'auto'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 63 $function$;
...
where p.is_active is true
  ...
  and lower(trim(coalesce(p.metadata->>'source_store_slug','')))='auto-kelly'
  and exists (
    select 1 from public.offers o
    join public.stores s on s.id=o.store_id and s.slug='auto-kelly'
