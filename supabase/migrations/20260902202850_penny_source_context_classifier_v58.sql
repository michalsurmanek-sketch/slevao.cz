create or replace function public.infer_product_filter_group_penny_context_v58(p_name text,p_quantity_text text)
returns text
language plpgsql
...
begin
  if n ~ '(^| )cappuccino( |$)' then return 'drinks'; end if;
  if n ~ '(kimchi|minidonut)' then return 'food'; end if;
  if n ~ '(^| )(water|waterr+)( |$)' and q ~ '(^| )[0-9]+([,.][0-9]+)? (ml|l)( |$)' then return 'drinks'; end if;
  return 'other';
end;
$function$;
...
set search_path to 'public','pg_temp'
as $function$ select 58 $function$;
  ...
  if v_is_penny then
    update public.products p
       set metadata=coalesce(p.metadata,'{}'::jsonb) || jsonb_build_object(
         ...
         'source_penny_context_source','offer-store-context-v58'
       )
     where p.id=new.product_id
       and coalesce(p.metadata->>'source_penny_context','false')<>'true';
  end if;
  return new;
end;
$function$;

drop trigger if exists trg_propagate_penny_product_context_v58 on public.offers;
create trigger trg_propagate_penny_product_context_v58
after insert or update of store_id,product_id on public.offers
for each row execute function public.propagate_penny_product_context_v58();
...
where exists (
  ...
  join public.stores s on s.id=o.store_id and s.slug='penny'
