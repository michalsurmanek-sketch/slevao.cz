  ...
  loop
    ...
    eligible_base as (
      ...
      where not rec.is_recipe or (
        ...
        and (lower(public.unaccent(rec.search_text)) not in ('brambory','cibule','cesnek','mrkev') or (
          exists (select 1 from unnest(regexp_split_to_array(s.candidate_text, '[^a-z0-9]+')) with ordinality as ctok(token, ord) where ctok.ord <= 3 and ctok.token = lower(public.unaccent(rec.search_text)))
        ...
        and (lower(public.unaccent(rec.search_text)) <> 'mleko' or s.candidate_text !~ '(kefir|acidofil|detske|kojeneck|beba|smetana)')
        and (lower(public.unaccent(rec.search_text)) not in ('olej','olej na smazeni') or (
    ...
    eligible as (
      select e.*, max(e.exact_count) over () as max_exact_count from eligible_base e
    ),
    amount_parsed as (
      select e.*, parsed.req, parsed.pkg,
      ...
      cross join lateral (
        select
          ...
          regexp_match(coalesce(e.offer->'products'->>'quantity_text',''), '([0-9]+([.,][0-9]+)?)\s*(kg|g|ml|l|ks)', 'i') as pkg
      ) parsed
    ),
    amount_scaled as (
      select a.*,
        case
          ...
          when lower(a.req[3]) in ('kg','g') and lower(a.pkg[3]) in ('kg','g') then case
            when a.variable_price then
            ...
            else greatest(1::numeric, ceil(
              (replace(a.req[1],',','.')::numeric * case when lower(a.req[3])='kg' then 1000 else 1 end) /
              nullif(replace(a.pkg[1],',','.')::numeric * case when lower(a.pkg[3])='kg' then 1000 else 1 end,0)
            )) end
          when lower(a.req[3]) in ('l','ml') and lower(a.pkg[3]) in ('l','ml') then case
            when a.variable_price then
          ...
          when lower(a.req[3])='ks' and lower(a.pkg[3])='ks' then greatest(1::numeric, ceil(replace(a.req[1],',','.')::numeric / nullif(replace(a.pkg[1],',','.')::numeric,0)))
          else 1::numeric
    ...
    final_candidates as (
      select a.*,
        case when rec.is_recipe and a.req is not null and a.pkg is not null then
          jsonb_set(
