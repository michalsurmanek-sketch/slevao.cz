-- Alias/name matching requires a brand or quantity signal; EAN remains authoritative.
...
begin
  ...
  if normalized_title = ''
     or not public.product_label_is_specific(p_title)
     or (
       nullif(btrim(coalesce(p_brand, '')), '') is null
       and nullif(btrim(coalesce(p_quantity, '')), '') is null
       and public.product_quantity_key(p_title) is null
     ) then
    return;
  end if;
