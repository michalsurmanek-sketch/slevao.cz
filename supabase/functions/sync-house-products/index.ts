...
const SOURCE = 'https://www.housebrand.com/cz/cz/sale';
const ADAPTER = 'house-official-clearance-v1';
...
function extractProducts(html: string) {
  ...
  if (start < 0 || end < 0) throw new Error('House strukturovaný seznam produktů nebyl nalezen.');
  return JSON.parse(html.slice(start + 'products: '.length, end));
}
function parseProducts(html: string, today: string) {
  const products = extractProducts(html);
  if (!Array.isArray(products)) throw new Error('House seznam produktů nemá očekávaný formát.');
  const rows: any[] = [];
  for (const product of products) {
    ...
    if (!id || !/^[A-Z0-9-]{5,}$/i.test(sku) || title.length < 3 || !href.startsWith('https://www.housebrand.com/cz/cz/') || !image.startsWith('https://static.housebrand.com/')) continue;
    if (product.has_discount !== true || product.finalPriceType !== 'clearance' || Number(product.minQty) !== 1 || merchant.name !== 'House' || merchant.isExternal !== false) continue;
    if (!Number.isFinite(price) || !Number.isFinite(oldPrice) || price < 10 || price > 10000 || oldPrice <= price) continue;
    rows.push({
      external_id: `house:${sku}`, title, normalized_title: normalize(title), price, old_price: oldPrice, quantity_text: null,
      valid_from: today, valid_to: addDays(today, 1), source_url: href, source_page: 1, product_id: null, image_url: image, confidence: 0.99,
      metadata: { adapter: ADAPTER, parser_version: ADAPTER, house_product_id: id, house_sku: sku, evidence: { official_price_type: 'clearance', current_price: price, regular_price: oldPrice, minimum_quantity: 1, merchant: 'House' } },
    });
  }
  const unique = [...new Map(rows.map((row) => [row.external_id, row])).values()];
  if (unique.length < 50 || unique.length > 80) throw new Error(`House parser našel ${unique.length} bezpečných produktů; očekáváno 50–80.`);
  return unique;
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: store, error: storeError } = await db.from('stores').select('id,name').eq('slug', 'house').single();
    if (storeError || !store) throw storeError || new Error('House nebyl nalezen.');
    ...
    if (existingSource) {
      ...
      const { error } = await db.from('leaflet_sources').insert({ store_id: store.id, name: 'House Výprodej', source_url: SOURCE, source_type: 'html', is_active: true, auto_publish: false, adapter_key: ADAPTER, extraction_strategy: 'structured_html' });
      if (error) throw error;
    }
    if (body.dry_run === true) return json({ ok: true, dry_run: true, publishable: rows.length, signature, candidates: rows });
    const { data: result, error: publishError } = await db.rpc('publish_structured_store_offers', { p_store_slug: 'house', p_adapter: ADAPTER, p_signature: signature, p_rows: rows, p_min_products: 50, p_max_products: 80, p_source_document_url: SOURCE, p_parser_version: ADAPTER });
    if (publishError) throw publishError;
    return json({ ok: true, store: store.name, published: rows.length, signature, result });
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : String(error), code: 'HOUSE_PRODUCTS_SYNC_FAILED' }, 500);
  }
});
