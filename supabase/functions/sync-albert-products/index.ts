...
function quantityKey(value: string | null | undefined) {
  return String(value || '').toLocaleLowerCase('cs').replace(/,/g, '.').replace(/[^0-9a-z.]+/g, '');
}
...
const GENERIC_BANNER = /^(?:A NATURE.?S PROMISE|Active PRO|Bag in Box|Barva|BARVY|Aviváž|Austrálie|Care Pánský|CENY|BĚŽNÁ CENA|Aplikace|Výběru|Při koupi)$/iu;
...
const BRAND_STOP = /(?:akční|nabídka|běžn|sleva|cena|aplikac|platí|různé|druhy|balení|obsah|kusů|procent|ušetříte|více|extra|pouze|výběru|novinka)/iu;
...
function allCaps(line: string) {
  const letters = line.replace(/[^A-Za-zÁ-ž]/gu, '');
  return letters.length >= 5 && letters === letters.toLocaleUpperCase('cs');
}
function containsQuantity(line: string) {
  return QTY_RE.test(String(line || '').replace(/\s*=.*$/u, ''));
}
function extractQuantity(lines: string[], titleIndex: number, priceIndex: number, title: string) {
  const candidates: string[] = [title];
  for (let i = titleIndex + 1; i < priceIndex; i++) candidates.push(lines[i]);
  for (let i = titleIndex - 1; i >= Math.max(0, titleIndex - 3); i--) candidates.push(lines[i]);
  for (const raw of candidates) {
    const line = clean(raw);
    if (!line || PRICE.test(line) || /(?:Kč|=\s*\d)/iu.test(line)) continue;
...
function stripTrailingQuantity(title: string) {
  const match = title.match(QTY_RE);
  if (!match || match.index === undefined) return title;
  const before = title.slice(0, match.index).replace(/[·,;:/\-–]+\s*$/u, '').trim();
  const after = title.slice(match.index + match[0].length).trim();
  return before && (!after || /^(?:balení|láhev|plech|PET|multipack)$/iu.test(after)) ? before : title;
}
function goodTitle(line: string) {
  if (!line || line.length < 3 || line.length > 90) return false;
  if (BAD.test(line) || START_BAD.test(line) || DESC.test(line) || ONLY_NUM.test(line) || GENERIC_BANNER.test(line)) return false;
  if (PRICE.test(line) || /Kč(?:\s|$)/iu.test(line) || /^[-–%]/u.test(line)) return false;
  if (containsQuantity(line) && norm(stripTrailingQuantity(line)).split(' ').filter(Boolean).length === 0) return false;
  if (allCaps(line) && !PRODUCT_WORD.test(line)) return false;
  if (/[,;:]\s*$/u.test(line)) return false;
  const words = norm(stripTrailingQuantity(line)).split(' ').filter(Boolean);
  if (words.length < 1 || words.length > 10) return false;
  const first = line.charAt(0);
  return first === first.toLocaleUpperCase('cs') && /[A-Za-zÁ-ž]/u.test(first);
}
function brandCandidate(line: string) {
  const value = clean(line);
  if (!value || value.length < 2 || value.length > 45) return false;
  if (BAD.test(value) || START_BAD.test(value) || DESC.test(value) || ONLY_NUM.test(value) || GENERIC_BANNER.test(value) || BRAND_STOP.test(value)) return false;
  if (PRICE.test(value) || /Kč/iu.test(value) || containsQuantity(value) || PRODUCT_WORD.test(value)) return false;
  const words = norm(value).split(' ').filter(Boolean);
  if (words.length < 1 || words.length > 4) return false;
  return /[A-Za-zÁ-ž]/u.test(value);
}
function findBrand(lines: string[], titleIndex: number) {
...
function lowSpecificity(title: string) {
  const n = norm(title);
  const tokens = n.split(' ').filter(Boolean);
  return LOW_SPECIFICITY.test(n) || (tokens.length <= 2 && PRODUCT_WORD.test(n));
}
function identityStrength(title: string, brand: string | null, quantity: string | null) {
  const tokenCount = norm(title).split(' ').filter(Boolean).length;
  if (brand && quantity) return 'strong';
  if (quantity && tokenCount >= 2 && !lowSpecificity(title)) return 'strong';
  if (quantity || brand || tokenCount >= 3) return 'medium';
  return 'weak';
}
...
function parsePage(text: string, page: number, document: any) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let k = titleIndex + 1; k < i; k++) {
      ...
      if (candidate !== rawTitle && goodTitle(candidate) && !containsQuantity(candidate)) {
        competing = true;
        break;
      }
    }
    ...
    if (brand && lowSpecificity(title) && !norm(title).startsWith(`${norm(brand)} `)) {
      title = `${brand} ${title}`.replace(/\s+/g, ' ').trim();
    }
    ...
    rows.push({
      ...
      product_id: null,
      image_url: null,
      confidence: strength === 'strong' ? 0.96 : 0.91,
...
async function resolveCatalog(rows: any[], albertStoreId: string) {
  ...
  for (let i = 0; i < names.length; i += 100) {
    ...
    const { data: productRows, error: pError } = await db
      .from('products')
      .select('id,normalized_name,brand,quantity_text,image_url,is_verified,image_verified')
      .in('normalized_name', chunk)
      .eq('is_active', true);
    if (pError) throw pError;
    products.push(...(productRows || []));

    const { data: aliasRows, error: aError } = await db
      .from('product_aliases')
      .select('product_id,normalized_alias,brand,quantity_text,source_store_id,confidence,products(image_url,is_active,is_verified,image_verified,brand,quantity_text)')
  ...
  for (const row of rows) {
    ...
    const aliasMatches = aliases.filter((a) =>
      a.normalized_alias === row.normalized_title
      && (String(a.source_store_id || '') !== albertStoreId || Number(a.confidence || 0) >= 0.98)
      && a.products?.is_active !== false
      && compatible(a.brand || a.products?.brand, a.quantity_text || a.products?.quantity_text, row)
    );
...
async function buildRows(documents: any[], storeId: string) {
  ...
  for (const document of documents) {
    ...
    for (const spread of spreads) {
      for (const page of Array.isArray(spread?.pages) ? spread.pages : []) {
        candidates.push(...parsePage(String(page?.text || ''), Number(page?.number || 0), document));
      }
    }
  }
  ...
  return {
    ...
    withImages: resolved.withImages,
    newStrict: resolved.newStrict,
  };
}
...
async function failure(storeId: string | null, sourceId: string | null, message: string) {
  ...
  if (storeId) {
    await db.from('store_product_sync_state').upsert({
      ...
      health_status: 'error',
      health_reason: 'Nová Albert v4 sada nebyla publikována; předchozí veřejná data zůstala beze změny.',
      updated_at: now,
    }, { onConflict: 'store_id' });
  }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (built.rows.length < MIN_SAFE || built.rows.length > MAX_SAFE) {
      throw new Error(`Albert v4 parser vytvořil ${built.rows.length} nabídek; bezpečný rozsah je ${MIN_SAFE}–${MAX_SAFE}.`);
    }

    const signature = await hash([
      ...documents.map((d: any) => `${d.source_hash}|${d.detected_valid_from}|${d.detected_valid_to}`),
      ...built.tokens,
      PARSER,
    ].sort().join('\n'));

    if (dryRun) {
      const strengths = built.rows.reduce((acc: Record<string, number>, row: any) => {
        acc[row.identity_strength] = (acc[row.identity_strength] || 0) + 1;
        return acc;
      }, {});
      return json({
        ok: true,
        ...
        strengths,
        signature,
        samples: built.rows.slice(0, 120).map((r) => ({
          title: r.title,
          brand: r.brand,
          quantity: r.quantity_text,
          identity_strength: r.identity_strength,
          price: r.price,
          matched: Boolean(r.product_id),
    ...
    if (!force && state?.last_source_signature === signature && state?.health_status === 'ok' && state?.parser_version === PARSER && Number(currentOffers || 0) >= MIN_SAFE) {
      return json({ ok: true, no_changes: true, parser: PARSER, store: 'Albert', current_offers: Number(currentOffers || 0), signature });
    }
