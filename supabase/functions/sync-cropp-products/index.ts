...
const SOURCE = 'https://www.cropp.com/cz/cz/vyprodej';
const ADAPTER = 'cropp-official-clearance-v1';
...
function extractProducts(html: string) {
  ...
  if (start < 0 || end < 0) throw new Error('Cropp strukturovaný seznam produktů nebyl nalezen.');
  return JSON.parse(html.slice(start + 'products: '.length, end));
}
function parseProducts(html: string, today: string) {
  const products = extractProducts(html);
  if (!Array.isArray(products)) throw new Error('Cropp seznam produktů nemá očekávaný formát.');
  const rows: any[] = [];
  for (const product of products) {
    ...
    if (!id || !/^[A-Z0-9-]{5,}$/i.test(sku) || title.length < 3 || !href.startsWith('https://www.cropp.com/cz/cz/') || !image.startsWith('https://static.cropp.com/')) continue;
    if (product.has_discount !== true || product.finalPriceType !== 'clearance' || Number(product.minQty) !== 1 || merchant.name !== 'Cropp' || merchant.isExternal !== false) continue;
    if (!Number.isFinite(price) || !Number.isFinite(oldPrice) || price < 10 || price > 10000 || oldPrice <= price) continue;
    rows.push({
      external_id: `cropp:${sku}`, title, normalized_title: normalize(title), price, old_price: oldPrice, quantity_text: null,
      valid_from: today, valid_to: addDays(today, 1), source_url: href, source_page: 1, product_id: null, image_url: image, confidence: 0.99,
      metadata: { adapter: ADAPTER, parser_version: ADAPTER, cropp_product_id: id, cropp_sku: sku, evidence: { official_price_type: 'clearance', current_price: price, regular_price: oldPrice, minimum_quantity: 1, merchant: 'Cropp' } },
    });
  }
  const unique = [...new Map(rows.map((row) => [row.external_id, row])).values()];
  if (unique.length < 150 || unique.length > 200) throw new Error(`Cropp parser našel ${unique.length} bezpečných produktů; očekáváno 150–200.`);
  return unique;
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Cropp HTTP ${response.status}`);
    ...
    const { data: store, error: storeError } = await db.from('stores').select('id,name').eq('slug', 'cropp').single();
    if (storeError || !store) throw storeError || new Error('Cropp nebyl nalezen.');
    ...
    if (existingSource) {
      ...
      const { error } = await db.from('leaflet_sources').insert({ store_id: store.id, name: 'Cropp Výprodej', source_url: SOURCE, source_type: 'html', is_active: true, auto_publish: false, adapter_key: ADAPTER, extraction_strategy: 'structured_html' });
      if (error) throw error;
    }
    if (body.dry_run === true) return json({ ok: true, dry_run: true, publishable: rows.length, signature, candidates: rows });
    const { data: result, error: publishError } = await db.rpc('publish_structured_store_offers', { p_store_slug: 'cropp', p_adapter: ADAPTER, p_signature: signature, p_rows: rows, p_min_products: 150, p_max_products: 200, p_source_document_url: SOURCE, p_parser_version: ADAPTER });
    if (publishError) throw publishError;
    return json({ ok: true, store: store.name, published: rows.length, signature, result });
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : String(error), code: 'CROPP_PRODUCTS_SYNC_FAILED' }, 500);
  }
});
