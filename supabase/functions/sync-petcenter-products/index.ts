...
const ADAPTER = 'petcenter-official-clearance-html-v1';
...
function parseProducts(html: string, today: string) {
  ...
  for (const block of blocks) {
    ...
    const relativeHref = decodeHtml(block.match(/<a href="([^"]+)" class="image"/)?.[1] || '');
    const priceText = block.match(/data-micro-price="([0-9.]+)"/)?.[1] || '';
    const oldText = block.match(/class="price-standard">[\s\S]*?<strong>([0-9.\s,]+)\s*Kč<\/strong>/)?.[1] || '';
    ...
    if (!block.includes('flag-vyprodej') || !block.includes('data-micro-availability="https://schema.org/InStock"')) continue;
    if (!/name="amount"[\s\S]{0,300}value="1"/.test(block)) continue;
    if (price == null || oldPrice == null || price < 1 || price > 30000 || oldPrice <= price || oldPrice > 50000) continue;
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (existingSource) {
      ...
      const { error } = await db.from('leaflet_sources').insert({ store_id: store.id, name: 'PetCenter Výprodej', source_url: SOURCE, source_type: 'html', is_active: true, auto_publish: false, adapter_key: ADAPTER, extraction_strategy: 'structured_html' });
    ...
    return json({ error: error instanceof Error ? error.message : String(error), code: 'PETCENTER_PRODUCTS_SYNC_FAILED' }, 500);
  }
});
