...
type OcrPage = { page_number:number; text_content:string; avg_confidence:number|string|null; words:OcrWord[]|null };
...
function parsePage(page: OcrPage): Candidate[] {
  ...
  for (const quantityLine of lines) {
    ...
    const priceWords = words.filter((w) => {
      ...
      return wordColumn === quantityLine.column && Number(w.top) >= quantityLine.top - 300 && Number(w.top) <= quantityLine.top - 20 && Number(w.height || 0) >= 18 && Number(w.confidence || 0) >= 55;
    });
    ...
    for (const word of priceWords) {
      for (const value of numericCandidates(clean(word.text))) {
        ...
        if (delta <= 0.06 && (!printed || delta < printed.delta)) printed = { value, text:clean(word.text), delta, confidence:Number(word.confidence || 0) };
      }
    }
    if (!printed) continue;
    ...
    out.push({
      ...
      raw_data: {
        parser:'terno-ocr-spatial-unit-price-v3', unit_price:unitPrice, unit_price_line:unitHit.line.text,
        unit_price_distance:round2(unitHit.distance), expected_price:expected, printed_price_word:printed.text,
        ...
        ocr_page_confidence:pageConfidence, coverage_label:'Vybrané prodejny Terno'
      }
    });
  }
  return out;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (dryRun) return json({ ok:true, dry_run:true, source_import_id:sourceImport.id, source_document_url:sourceImport.source_document_url, candidate_count:candidates.length, candidates:candidates.slice(0,80) });
    ...
    const hash = `terno-ocr-safe-v3-${sourceImport.id}`;
    ...
    if (!derivedId) {
      const created = await db.from('leaflet_imports').insert({
        ...
        confidence:0.98,
        metadata:{ parser:'terno-ocr-spatial-unit-price-v3', deterministic:true, verified_pipeline:true, source_import_id:sourceImport.id, coverage_label:'Vybrané prodejny Terno', region:sourceImport.metadata?.region || null }
      }).select('id').single();
